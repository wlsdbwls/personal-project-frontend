<!-- <template lang="">
    <v-container class="container">
      <div class="restaurantModifyCard" justify-center>
      <v-card-text>
        <form @submit.prevent="onSubmit">
        <table class="restaurantModifyTable">
          <tr>
            <td style="font-weight: bold">상호명</td>
            <td>
                <v-text-field type="text" class="inputValue" v-model="modifiedRestaurantName"/>
            </td>
          </tr>
          <tr>
            <td style="font-weight: bold">음식점 정보</td>
            <td>
                <v-textarea class="inputValue" rows="4" v-model="modifiedRestaurantInfo"/>
            </td>
          </tr>
        </table>
      <div>
...
<template lang="">
    <div>
      <div class="contents_box1">
        <h2 class="contents_name">맛집 수정</h2>
      </div>
            ...
            <ul class="edit2_form_list1">
                ...
                <li>
                  ...
                  <router-link class="cancel-link" :to="{ name: 'RestaurantListPage' }">취소</router-link> -->
                   <div>
                    <v-btn @click="modifyBtn" class="valid_form_button text-center">수정완료</v-btn>
                    <router-link class="cancel-link" :to="{
                            name: 'BusinessRegisteredRestaurantsPage',
                            params: { id: this.filteredRestaurant.id }
                        }">취소</router-link>
                </div>
...
export default {
    ...
    created() {
        this.restaurantName = this.filteredRestaurant.restaurantName
        this.restaurantInfo = this.filteredRestaurant.restaurantInfo
        this.restaurantNumber = this.filteredRestaurant.restaurantNumber
        this.foodType = this.filteredRestaurant.foodType

        // this.menuItems = this.restaurant.menuItems.map(item => ({ menuItem: item.menuItem, menuPrice: item.menuPrice }));
        this.menuItem = this.filteredRestaurant.menuItem
        this.menuPrice = this.filteredRestaurant.menuPrice

        this.imageUrls = this.filteredRestaurant.imageUrls;
        // this.imagePreviews = this.restaurant.imageUrls.map(url => url);
    },
    methods: {
        async onSubmit() {
