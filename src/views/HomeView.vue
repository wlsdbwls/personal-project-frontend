            ...
            <v-flex xs12 sm6 md4 lg3 xl2>
              <v-card @click="handleCellClick(item)">
                <div class="restaurant-image-container" v-if="item.restaurantImagePath">
                  <v-img :src="getS3ImageUrl(item.restaurantImagePath)" aspect-ratio="1" class="restaurant-image"></v-img>
                </div>
                ...
                <v-card-actions>
                  ...
                  <div class="heart-icon-container">
                    <!-- <template v-if="isLiked(item.id)">
                      <v-icon class="mdi mdi-heart" style="color: red;" @click.stop="toggleLike(item.id)"></v-icon>
                    </template>
                    <template v-else>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      searchKeyword: state => state.restaurantModule.searchKeyword, // Vuex 스토어의 searchKeyword 상태를 가져옴
    }),
  }
}
</script>
